...
const MODULES = [
  ...
  },
  {
    ...
    progress: 100,
    status: 'complete',
    summary: '3 modos de evaluación (Dictation/Vocabulary Quiz/Combined) con 6 tipos de pregunta. TTS automático + IA generadora + antitrampa 6 capas + corrección instantánea + Telegram. Sala de Control con 3 paneles RT. Envío de códigos por email + prueba a correos del docente. Volumen por pregunta. Bloqueo de traducción/paste/autocomplete.',
    works: [
      'DictationPage: shell de tabs → 8 componentes extraídos en src/components/dictation/',
      'CreateTab: wizard 3 pasos + assessmentMode selector + panel post-publish con link, envío a estudiantes y prueba',
      'CreateTab: progreso visual IA (mensajes rotatorios) + barra TTS determinada por item',
      'CreateTab: botón 🧪 prueba email al docente — extra_email persiste en localStorage; código TEST- funcional',
      'ListTab: biblioteca de dictados con filtros + botones 🗑️ eliminación cascade (responses→results→instances→sessions)',
      ...
      'ASSESSMENT_MODES / ITEM_COUNTS / SECTION_META / getQuestionCounts / scoreWriting / scoreDictation',
      'ManualEntryForm: 6 secciones incluyendo listen_comprehension (audio_text + 4 opciones)',
      'VocabSetPicker + VocabLibraryTab: CRUD vocabularios reutilizables con RLS',
      'AudioExportPanel: descarga individual MP3 + ZIP completo con JSZip',
      'exportDictationHtml.js: PDF por modo + renderers 6 tipos + answer key + buildCorrectedHtml/printCorrectedHtml',
      'DictationPlayerPage: player público con antitrampa 6 capas + Broadcast listener teacher→student',
      'DictationPlayerPage: volumen independiente por pregunta — Web Audio API GainNode + crossOrigin="anonymous" (funciona en iOS)',
      'DictationPlayerPage: paste bloqueado a nivel document (registra violación) + translate="no" + notranslate',
      'DictationPlayerPage: autoComplete/autoCorrect/autoCapitalize/spellCheck desactivados en todos los inputs',
      'DictationPlayerPage: MC cyclic placement (A→B→C→D) — previene inducción por patrón',
      'DictationPlayerPage: submitted phase robusto — fallback chain + spinner + timeout 12s DB poll',
      'DictationPlayerPage: teacher_warning overlay 10s + force_close → fase force_closed permanente',
      'SessionControlPage: Sala de Control 3 paneles RT + badges violación titilantes hasta reconocer',
      'SessionControlPage: CorrectedExamView modal + PDF corregido + email representante',
      'SessionControlPage: sección "🧪 Probar envío a mis correos" en panel izquierdo con error detail',
      'WarningModal + DictationPreview (read-only blueprint con correct_answers)',
      'dictationAI.js: generateDictation() — prompt dinámico por assessmentMode, 6 tipos',
      'dictation-tts Edge Function: Azure Cognitive Services TTS → MP3 → Supabase Storage',
      'dictation-corrector Edge Function: corrección server-side + 0-responses → 1.0 + Telegram',
      'dictation-send-codes Edge Function: un email por estudiante con código + URL visible (FROM: onboarding@resend.dev)',
      'dictation-send-test Edge Function: prueba a correos del docente + instancia TEST- reutilizable (instance_status: ready)',
      'dictation-notify Edge Function: email al representante con nota y desglose por sección',
      'dictation_vocab_sets: tabla DB para vocabularios reutilizables con RLS',
      'Service Worker v2: cache-first para assets, bypass para Supabase/non-cacheable',
    ],
    pending: [
      'Azure TTS API key — requiere configuración en Supabase secrets',
    ],
    history: [
      { date: '2026-05-28', reason: 'Email fix + Web Audio API volume + test email Sala de Control', detail: 'Fix emails: FROM cambiado a onboarding@resend.dev (dominio Resend pre-verificado) → resuelve 21/21 fallos. Fix instance_status: pending→ready en dictation-send-test (pending no existe en CHECK constraint). Fix volumen iOS: Web Audio API GainNode + crossOrigin="anonymous" en <audio> (sin esto createMediaElementSource lanza SecurityError silencioso). Sección "🧪 Probar envío" añadida a SessionControlPage (panel izquierdo). Error detail expandido en send-codes y send-test UIs.' },
      { date: '2026-05-28', reason: 'Anti-trampa hardening + UX mejoras', detail: 'Volume control por pregunta (slider+icono). Paste bloqueado a documento (registra violación). translate="no" + notranslate + <meta google notranslate> on-mount. autoCorrect/autoCapitalize/spellCheck off. Corrector robusto: 0-responses → 1.0. Fallback chain submitted phase + spinner + timeout 12s. dictation-send-test Edge Fn (prueba a correos docente). URL visible en emails. Botón 🗑️ eliminar sesiones/blueprints. MC cyclic placement. Badge violaciones titilante.' },
