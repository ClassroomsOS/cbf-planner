...
The six **CBF sections** per day always follow this order and default times: `subject/TOPICS (~8 min)`, `motivation/SUBJECT TO BE WORKED (~7 min)`, `activity/MOTIVATION (~10 min)`, `skill/SKILL DEVELOPMENT (~25 min)`, `assignment/ASSIGNMENT (~3 min, optional)`, `closing/CLOSING (~5 min)`. The biblical verse of the indicator permeates the entire day: reminded in MOTIVATION, connected in SKILL DEVELOPMENT, reflected upon in CLOSING. Class rules are always stated in TOPICS.
